...
def getMirrorHash():
   ...
   # workaround:
   import subprocess
   s = subprocess.Popen('find -L www/mirror/ -type f -and \( -size +10M -not -path "*src*" -not -path "*devel*" -not -path "*meta*" -not -name "*.spkg" \)  -exec ls -la {} \; | md5sum | cut -d" " -f1', shell=True, stdout=subprocess.PIPE)
   return s.stdout.readline().split('\n')[0]

def getSpkgHash():
  import subprocess
  s = subprocess.Popen('find -L www/mirror/ -type f -and \(  \( -path "*src*" -and -size +100M \)  -or \( -path "*standard*" -and -name "*.spkg" -or -name "*install" \) \)  -exec ls -la {} \; | md5sum | cut -d" " -f1', shell=True, stdout=subprocess.PIPE)
  return s.stdout.readline().split('\n')[0]
...
def getVersion():
   ...
   for l in open(os.path.join('www', 'inc', 'variables.shtml')):
      ...
      if not m == None and len(m.groups()) >= 1:
         ver = m.group(1)
...
#      msg += " / " + " ".join(sys.argv[1:])
   ...
   stampfiles = [ os.path.join('.','www','mirror','zzz','timestamp.html'), os.path.join('..','www2','mirror','zzz','timestamp.html')]
