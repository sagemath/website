...
TARGETS = [ './www/inc/mirrorselector.shtml', '../www2/inc/mirrorselector.shtml']
TARGETS_SPKG = [ './www/inc/mirrorselector-src.shtml', '../www2/inc/mirrorselector-src.shtml']
TARGETS_LIST = [ './www/mirror_list', '../www2/mirror_list']
OUTPUT_FILE = './mirror_manager.out'
METALINK_FILE = [ './www/metalink.helper', '../www2/metalink.helper' ]
TORRENT_FILE  = [ './www/torrent.helper', '../www2/torrent.helper' ]
...
class Mirror():
  ...
  def entry(self):
    ...
    return '<a href="%s<!--#echo var="mirrordir"-->index.html">%s %s</a>\n' % (self.url, f, n)
...
# intro text
# The current release is <!--#echo var="version"-->.
TS_INTRO = r"""
...
def mirrors_html():
  ...
  # copy parts of the existing page
  HEADER = ""
  FOOTER = ""
  state = 0
  re_start = re.compile(r'STARTTOKEN')
  re_end   = re.compile(r'ENDTOKEN')
  MH = open(MIRRORS_HTML)
  try:
    for l in MH:
      if state == 0:
        if not re_start.search(l):
          HEADER += l
        else:
          state = 1
          #print "state 1"
          HEADER += l # adds token 1
      elif state == 1:
        if re_end.search(l):
          FOOTER += '\n' + l # adds token 2 with newline
          # without newline, it might move more and more to the right due to space or other crap
          state = 2
          #print "state 2"
      elif state == 2:
        FOOTER += l
  ...
  # stick all three parts together
  if state == 0:
    return 'error: please <a href="./contact.html">contact site admin</a>.'
  ...
  def fetch_task(mirror):
    ...
    if True: #while True:
       ...
       try:
         ...
         #response = subprocess.Popen(['curl', '-m 30' ,mirror.url + TIMESTAMP_SUFFIX], stdout=subprocess.PIPE)
         ret[mirror] = response.read()
         #ret[mirror] = response.stdout.read()
         OUTPUT +=  '%8.2f [ms] %s\n' % ((time.time() - time1) * 1000.0, mirror.name)
         #break
       except Exception, err: # urllib2.URLError, err:
         #err_count += 1
         #print "err_count", err_count
         #time.sleep(1)
         if True: #err_count > 5:
           OUTPUT +=  '%8.2f [ms] %s' % ((time.time() - time1) * 1000.0, mirror.name)
           OUTPUT += " -> %s\n" % str(err)
           #OUTPUT += ' ' + str(err)
           #break
...
def build_mirrorselector(mirrors, TS):
  """
  builds the mirrorselector.shtml page, see sample at the bottom
  """
  global OUTPUT
  page = TS_INTRO 
  global TOTAL_NUMBER
...
if __name__ == '__main__':
  ...
  new_mirrors_html = mirrors_html()
  MH = open(MIRRORS_HTML, 'w')
  try:
    MH.write(new_mirrors_html) 
  finally:
    MH.close()
  TS = fetch_timestamps() 
