var navbarjs = {
   ...
   init: function() {
      ...
      for(var i = 0; i <= aArr.length; i++) {
        ...
        if(i < aArr.length) {
         var idxhtml = aArr[i].href.indexOf("html");
         if (idxhtml == -1) idxhtml = aArr[i].href.length;
         if(document.location.href.indexOf(aArr[i].href.substring(0,idxhtml-1))>=0) {
            navbarjs.activeTab = navbarjs.currentTab = i;
            aArr[i].setAttribute("class", "active");
            break;
         }
        } else {//the additional last i index does not exist, then don't show animation at all
          return;
        }
      }
     };
